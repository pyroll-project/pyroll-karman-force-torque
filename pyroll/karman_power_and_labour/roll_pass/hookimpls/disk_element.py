...
def direction(self: RollPass.DiskElement):
    ...
    return 2 / np.pi * np.arctan(
        (self.roll_pass.velocity - self.velocity) / Config.SHEAR_STRESS_DIRECTION_REGULARISATION)
...
def longitudinal_stress_increment(self: RollPass.DiskElement):
    ip = self.in_profile
    return -(
            ip.longitudinal_stress * self.equivalent_height_derivative + 2 * ip.longitudinal_contact_friction - 2 * ip.normal_stress * np.tan(
        ip.longitudinal_angle)) * self.length / ip.equivalent_height


@RollPass.DiskElement.OutProfile.longitudinal_stress
def longitudinal_stress(self: RollPass.DiskElement.OutProfile):
    return self.disk_element.in_profile.longitudinal_stress + self.disk_element.longitudinal_stress_increment
...
def longitudinal_contact_friction(self: RollPass.DiskElement.OutProfile):
    ...
    coulombs_normal_pressure = - self.normal_stress / (1 + self.roll_pass.coulomb_friction_coefficient * np.tan(
        self.longitudinal_angle) * self.disk_element.direction)
